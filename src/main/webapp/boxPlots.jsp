...
<body>
	...
	<script>
		var margin = {
			...
			bottom : 20,
			left : 50
		}, width = 120 - margin.left - margin.right, height = 500 - margin.top
				- margin.bottom;
		...
		d3.csv("boxPlots.csv",
						function(error, csv) {
							...
							csv.forEach(function(x) {
								var e = Math.floor(x.Expt - 1), r = Math
										.floor(x.Run - 1), s = Math
										.floor(x.Speed), d = data[e];
								if (!d)
									d = data[e] = [ s ];
								else
									d.push(s);
		...
		function iqr(k) {
			return function(d, i) {
				var q1 = d.quartiles[0], q3 = d.quartiles[2], iqr = (q3 - q1)
						* k, i = -1, j = d.length;
				while (d[++i] < q1 - iqr)
					;
				while (d[--j] > q3 + iqr)
